...
def enumerate_candidates(elements, nominal_mass, max_atoms=4, polarity=-1,
                         mz_tol=0.05, nominal_tol=0.6, ignore_abundance=False):
    ...
    ``ignore_abundance`` drops the natural-abundance term from the plausibility
    score (plausibility becomes size-penalty only). Use it for isotopically
    labelled samples, where natural abundances are not a valid prior — a
    15N/13C tracer can drive a "rare" ion to be the dominant peak.
    """
    atom_list = []
    for sym in elements:
        el = _BY_SYM.get(sym)
...
class SeparationMatch:
    ...
    rms_mau: float = 0.0
    score: float = 0.0                           # rms + plausibility penalty


def match_by_separation(observed_mz, expected_elements, polarity=-1,
                        max_atoms=4, top_anchors=8, plaus_weight=0.0):
    ...
    shift the candidate set onto the data and score the anchoring. The score
    combines separation fit and anchor plausibility::

        score = RMS_mau + plaus_weight * (-log10(plausibility))

    ``plaus_weight`` is in mau per decade of implausibility: 0 reproduces pure
    separation-RMS minimisation; larger values prefer chemically sensible
    anchors even at some cost in geometric fit. Returns the lowest-score match.
    """
    ...
    for anchor in cands[:max(top_anchors, 1)]:
        ...
        penalty = plaus_weight * (-math.log10(max(anchor.plausibility, 1e-12)))
        score = rms + penalty
...
class PeakAssignment:
    peak_index: int
    observed: float
    delta_obs_mau: float      # separation from the reference peak (nominal)
    candidate: Candidate
    delta_theo_mau: float     # candidate separation from the reference species
    err_mau: float            # (candidate - predicted) on the difference axis


def _candidates_for(peaks_mz, elements, polarity, max_atoms, nominal_mass,
                    ignore_abundance):
    if nominal_mass is None:
        nominal_mass = round(sum(peaks_mz) / len(peaks_mz))
    return enumerate_candidates(elements, nominal_mass, max_atoms=max_atoms,
                                polarity=polarity, mz_tol=0.1,
                                ignore_abundance=ignore_abundance), nominal_mass


def identify_by_difference(peaks_mz, ref_index, ref_formula, elements,
                           polarity=-1, max_atoms=4, nominal_mass=None,
                           ignore_abundance=False):
    """Identify peaks by mass *difference* from one known reference peak.

    The user tags peak ``ref_index`` as ``ref_formula``. With scale assumed 1
    (no absolute calibration), each peak's true mass is
    ``M_ref + (peak - ref_peak)``, and the nearest candidate ion identifies it.
    Returns a list of PeakAssignment, or None if the reference formula isn't
    among the enumerated candidates.
    """
    peaks = [float(p) for p in peaks_mz]
    cands, _ = _candidates_for(peaks, elements, polarity, max_atoms,
                               nominal_mass, ignore_abundance)
    ref_cand = next((c for c in cands if c.formula == ref_formula), None)
    if ref_cand is None or not (0 <= ref_index < len(peaks)):
        return None
    m_ref = ref_cand.mz
    ref_obs = peaks[ref_index]
    out = []
    for i, o in enumerate(peaks):
        predicted = m_ref + (o - ref_obs)            # scale = 1
        near = min(cands, key=lambda c: abs(c.mz - predicted))
        out.append(PeakAssignment(
            peak_index=i, observed=o, delta_obs_mau=(o - ref_obs) * 1000.0,
            candidate=near, delta_theo_mau=(near.mz - m_ref) * 1000.0,
            err_mau=(near.mz - predicted) * 1000.0))
    return out


@dataclass
class BlindSolution:
    assignment: list          # one Candidate per observed peak
    rms_mau: float            # separation-pattern fit
    mean_plausibility: float


def identify_blind(peaks_mz, elements, polarity=-1, max_atoms=4,
                   nominal_mass=None, ignore_abundance=False, top_n=5):
    """Best-guess identification with no known peak, using only differences.

    Tries every candidate ion as the identity of one peak (which fixes an
    offset, scale 1), assigns the remaining peaks to their nearest candidates,
    and scores the whole assignment by separation RMS. Distinct assignments are
    ranked best-first. The caller should show several so the RMS *gap* between
    the best and the next is visible — that gap, not the absolute RMS, is the
    real confidence signal, and it collapses if too many elements are in play.
    """
    peaks = [float(p) for p in peaks_mz]
    if not peaks:
        return []
    ...
    for c in cands:
        for p in peaks:
            ...
            for o in peaks:
                tgt = o - offset
                near = min(cands, key=lambda k: abs(k.mz - tgt))
                sq += (near.mz - tgt) ** 2
                assign.append(near)
            rms = (sq / len(peaks)) ** 0.5 * 1000.0
            key = tuple(a.formula for a in assign)
            if key not in best_by_key or rms < best_by_key[key].rms_mau:
                mp = sum(a.plausibility for a in assign) / len(assign)
                best_by_key[key] = BlindSolution(assignment=assign, rms_mau=rms,
                                                 mean_plausibility=mp)
    sols = sorted(best_by_key.values(), key=lambda s: s.rms_mau)
    return sols[:top_n]
