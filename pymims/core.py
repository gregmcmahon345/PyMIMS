...
def _json_default(o):
    """JSON fallback for numpy scalar / array types that may appear in the
    metadata dict, so a session's metadata serialises cleanly."""
    ...
    if isinstance(o, (bytes, bytearray)):
        return o.decode('ascii', errors='replace')
    raise TypeError(f"Not JSON serializable: {type(o)!r}")


def _bin_sum(arr2d, n):
    """Sum-bin a 2-D array into n x n blocks.

    Crops the trailing rows / columns that don't fill a complete block,
    independently in height and width, so arbitrary rectangular (non-square,
    non-power-of-two) images are handled cleanly. At most n-1 rows and n-1
    columns are discarded, only at the far/bottom-right edge.

    Summing (not averaging) is used deliberately: for NanoSIMS this pools the
    SAME set of pixels for every channel, so binning numerator and denominator
    and then dividing (Sum(A)/Sum(B)) preserves the same-analytical-volume
    correspondence that simultaneous multicollection provides. It is simply a
    larger pixel. (Median-binning each channel independently would NOT — it
    selects a different representative pixel per channel — so never do that.)

    n <= 1 returns the array unchanged. A factor larger than a dimension also
    returns the array unchanged (guards against an empty result).
    """
    n = int(n)
    if n <= 1:
        return arr2d
    ...
    if hc == 0 or wc == 0:
        return arr2d
    cropped = arr2d[:hc, :wc]
    return cropped.reshape(hc // n, n, wc // n, n).sum(axis=(1, 3))
...
class MimsImage:
    ...
    def drop_planes(self, indices, verbose=True):
        """
        Manually drop specific planes from the stack by index.

        Companion to ``auto_drop_bad_planes`` for planes spotted by eye — in
        the diagnostics scan or by scrubbing the stack — that a percentage
        threshold misses, or to remove planes a threshold would over-flag.
        Behaviour matches ``auto_drop_bad_planes`` exactly: removal is
        destructive and IN PLACE (re-load the file to recover), ``self.data``
        shrinks, ``self.metadata['n_planes']`` is kept in sync, any existing
        drift correction is invalidated (so it is re-run on the new stack),
        and the dropped indices are appended to ``self._dropped_planes`` for
        provenance.

        Run BEFORE drift correction, for the same reason auto-drop does: bad
        planes are the principal cause of silent drift-correction failures.

        Parameters
        ----------
        indices : int or sequence of int
            Plane index/indices (into the CURRENT stack) to remove.
            Out-of-range values and duplicates are ignored (with a note when
            ``verbose``); dropping every remaining plane is refused.
        verbose : bool, default True
            Print what was dropped.

        Returns
        -------
        list of int — the plane indices actually dropped (current-stack
        coordinates, matching ``auto_drop_bad_planes``).

        Note
        ----
        Like ``auto_drop_bad_planes``, the indices recorded in
        ``self._dropped_planes`` are CURRENT-stack coordinates at the time of
        each drop, so after successive drops they are not original-file
        indices. Session-save (provenance) should map these back to original
        coordinates when that feature is built.
        """
        if self.data is None:
            raise RuntimeError("Image data not loaded.")
        ...
        requested = sorted({int(i) for i in indices})
        out_of_range = [i for i in requested if i < 0 or i >= n_planes]
        drop = [i for i in requested if 0 <= i < n_planes]
        if out_of_range and verbose:
            print(f"Ignoring out-of-range plane index/indices {out_of_range} "
                  f"(stack has {n_planes} planes; valid 0..{n_planes - 1}).")
        if not drop:
            if verbose:
                print("No valid planes to drop.")
            return []
        if len(drop) >= n_planes:
            raise ValueError(
                f"Refusing to drop all {n_planes} plane(s) — nothing would "
                f"remain. Drop fewer planes or re-load the file.")

        keep_mask = np.ones(n_planes, dtype=bool)
        ...
        self.metadata['n_planes'] = int(self.data.shape[0])
        if self.corrected is not None:
            # Drift correction was already applied; invalidate it so the user
            # knows to re-run drift_correct on the new (shorter) stack.
            self.corrected = None
            self.shifts = None
            if verbose:
                print("Note: dropping planes invalidated previous drift "
        ...
        if not hasattr(self, '_dropped_planes'):
            self._dropped_planes = []
        self._dropped_planes.extend(drop)

        if verbose:
            print(f"Dropped {len(drop)} plane(s): {drop}")
            print(f"  Stack: {n_planes} → {self.data.shape[0]} planes.")
        return drop
    ...
    def ratio(self, numerator, denominator, corrected=True,
              min_counts=None, max_rel_err=None, sum_planes=True, bin=1):
        """
        ...
        # Sum-bin numerator and denominator over the SAME blocks, then divide.
        # This enlarges the analytical volume (a bigger pixel) while preserving
        # the same-volume correspondence; the Poisson error below is then
        # correct for the pooled counts. Default bin=1 is a no-op.
        if sum_planes and bin and int(bin) > 1:
            A = _bin_sum(A, bin)
            B = _bin_sum(B, bin)
        ...
        return {
            ...
            'bin'      : int(bin) if (bin and sum_planes) else 1,
        }
    ...
    def plot_ratio(self, numerator, denominator, corrected=True,
                   ...
                   percentile=(1, 99), bin=1, outpath=None, show=None):
        """
    ...
    def plot_hsi(self, numerator, denominator, corrected=True,
        ...
        # Optional cosmetic de-speckle of the HSI hue: median-filter the RATIO
        # image AFTER it has been formed (NaN-aware), never the mass images.
        # Each pixel's ratio is already a same-volume ratio; this only smooths
        # the spatial map of valid ratios for display. The underlying data and
        # the quantitative Ratio-tab panels are unaffected.
        if median_smooth and int(median_smooth) > 1:
            from scipy.ndimage import generic_filter
            k = int(median_smooth)
            R = generic_filter(R, np.nanmedian, size=k, mode='nearest')
    ...
    def read_full_metadata(self, populated_slots=(1, 2, 3, 6, 7)):
        """
        Parse the full Cameca header from the cached header bytes.

        Returns the structured dict produced by metadata.parse_full_metadata:
        sub-dicts for 'basic', 'bfield', 'primary', 'secondary', 'selectors',
        'mass_table', 'detectors', 'pending', and a 'layout' compatibility
        report. The mass table is keyed by channel index (matching
        self.masses order) via the nominal-mass match.

        Parameters
        ----------
        populated_slots : tuple of int
            Detector slot numbers physically installed, used only for the
            HV/threshold detector-parameters table. Default (1, 2, 3, 6, 7)
            matches NS50L geometry.
        """
        return metadata.parse_full_metadata(
            self._raw_header,
            nominal_masses=self.nom_masses,
            populated_slots=populated_slots,
        )

    def print_metadata(self, populated_slots=(1, 2, 3, 6, 7)):
        """Pretty-print the full header (mass table, detectors, slits, columns)."""
        meta = self.read_full_metadata(populated_slots=populated_slots)
        print(metadata.format_metadata(meta, masses=self.masses))

    # ── Session save / load ────────────────────────────────────────────────────

    _SESSION_FORMAT_VERSION = 1

    def save_session(self, path, verbose=True):
        """
        Save the current working state to an (uncompressed) .npz session file.

        Bundles everything needed to resume analysis WITHOUT the original .im:
        the raw stack, the drift-corrected stack and shifts (if present), the
        mass labels and nominal masses, the full metadata dict, the raw Cameca
        header bytes (so the Metadata tab still works after reload), the
        dropped-plane record, and a provenance block (format version, save
        time, original path, drift state).

        Display preferences — binning, contrast, colourmap, display filters —
        are deliberately NOT saved: they are view settings, not data. Stacks
        are stored at full resolution and binning is re-applied at view time.

        Uncompressed np.savez is used on purpose (decided over HDF5/gzip): load
        speed matters more than file size for this workflow.

        Parameters
        ----------
        path : str
            Output path; ``.npz`` is appended if absent.
        verbose : bool, default True
        ...
        """
        if self.data is None:
            raise RuntimeError("No data loaded; nothing to save.")

        dropped = list(map(int, getattr(self, '_dropped_planes', [])))

        arrays = {
            'data':           self.data,
            'masses':         np.asarray(self.masses),                  # unicode array
            'nom_masses':     np.asarray(self.nom_masses, dtype=float),
            'dropped_planes': np.asarray(dropped, dtype=np.int64),
        }
        if self._raw_header is not None:
            arrays['raw_header'] = np.frombuffer(self._raw_header, dtype=np.uint8)
        if self.corrected is not None:
            arrays['corrected'] = self.corrected
        if self.shifts is not None:
            arrays['shifts'] = np.asarray(self.shifts)

        provenance = {
            'pymims_session_version': self._SESSION_FORMAT_VERSION,
            'saved': datetime.datetime.now(datetime.timezone.utc)
                              .isoformat(timespec='seconds'),
            'original_path': self.path,
            'n_planes': int(self.data.shape[0]),
            'n_masses': int(self.data.shape[1]),
            'drift_applied': self.corrected is not None,
            'dropped_planes': dropped,
        }
        # Metadata and provenance ride along as 0-d JSON string arrays so the
        # file stays allow_pickle=False safe (no object arrays).
        arrays['meta_json'] = np.asarray(json.dumps(self.metadata,
        ...
        if verbose:
            ...
            print(f"Saved session: {path}  ({mb:.1f} MB, "
                  f"{provenance['n_planes']} planes, "
                  f"drift {'applied' if provenance['drift_applied'] else 'none'})")
        return path

    @classmethod
    def load_session(cls, path, verbose=True):
        """
        Reconstruct a MimsImage from a .npz session written by save_session —
        WITHOUT needing the original .im file.

        The returned object behaves like a freshly loaded image: single
        channel, ratio, HSI, drift status and the full Metadata tab (via the
        restored raw header) all work. ``self.path`` is set to the original
        file path recorded at save time (for titles and provenance), even if
        that file is not present on this machine.

        Uses ``cls.__new__`` to build the instance directly, bypassing
        ``__init__`` (which would insist on reading a real .im).

        Parameters
        ----------
        path : str
            Path to a .npz session file.
        verbose : bool, default True
        ...
        MimsImage
        """
        obj = cls.__new__(cls)        # bypass __init__: no .im read required

        with np.load(path, allow_pickle=False) as npz:
            files = set(npz.files)
            if 'data' not in files:
                raise ValueError(
                    f"'{path}' is not a PyMIMS session file (no 'data' array).")
            obj.data        = npz['data']
            obj.corrected   = npz['corrected'] if 'corrected' in files else None
            obj.shifts      = npz['shifts']    if 'shifts'    in files else None
            obj._raw_header = (npz['raw_header'].tobytes()
                               if 'raw_header' in files else None)
            obj.masses      = ([str(m) for m in npz['masses']]
                               if 'masses' in files else [])
            obj.nom_masses  = (npz['nom_masses'].tolist()
                               if 'nom_masses' in files else [])
            obj._dropped_planes = (npz['dropped_planes'].tolist()
                                   if 'dropped_planes' in files else [])
            meta = (json.loads(str(npz['meta_json'].item()))
                    if 'meta_json' in files else {})
            prov = (json.loads(str(npz['provenance_json'].item()))
                    if 'provenance_json' in files else {})

        obj.metadata = meta
        obj._session_provenance = prov
        obj.path = prov.get('original_path', path)
        # Keep metadata['n_planes'] consistent with the restored stack (it may
        # have shrunk via plane drops before the session was saved).
        obj.metadata['n_planes'] = int(obj.data.shape[0])

        if verbose:
            print(f"Loaded session: {path}")
            print(f"  Original file : {obj.path}")
            print(f"  Stack         : {obj.data.shape[0]} planes × "
                  f"{obj.data.shape[1]} channels  |  drift "
                  f"{'applied' if obj.corrected is not None else 'none'}")
            if prov.get('saved'):
                print(f"  Saved         : {prov['saved']}")
        return obj
