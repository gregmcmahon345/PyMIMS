...
def help_view():
    ...
    hmr_md = (
        "High-mass-resolution peak analysis from an exported spectrum "
        "(`.csv` / `.xlsx`).\n\n"
        "**Spectrum** — load and view; toggle x-axis units and log-y.\n\n"
        "**IRF & resolving power** — pick the rising/falling edge and a reference "
        "mass; **MRP** is computed from the 10–90 % edge width. (This is the "
        "metric the V2 ion-optics tab will go *beyond* — MRP lives between 10 % and "
        "90 %, so it's blind to peak shape, tails and asymmetry.)\n\n"
        "**Deconvolution** — separates overlapping peaks; choose the method and "
        "the n-component range, grid, stability and minimum-SNR controls, then "
        "read the component table and residuals.\n\n"
        "**Species identification** — click the elements you expect in the sample "
        "on the periodic table, set polarity and max atoms, and identify by "
        "*relative* peak separation. The amu axis is the instrument's nominal "
        "scale (no absolute calibration), so identification uses relative gaps "
        "only. *Reference peak* mode: tag one known peak and read the rest off by "
        "mass difference. *Blind* mode: best guess from the separation pattern — "
        "trust it only when the RMS gap to the next assignment is wide. The "
        "labeled-sample box drops natural abundances from the ranking.\n\n"
