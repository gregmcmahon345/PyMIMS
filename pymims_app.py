...
def draw_roi_view(img):
    ...
    pal = list(Greys256)[::-1]            # low = black, high = white (signal bright)
    ...
    def _rasterise(event):
        ...
        for src, prefix in ((free_src, "lasso"), (poly_src, "poly")):
            xs_all = list(src.data.get("xs", []))
            ys_all = list(src.data.get("ys", []))
        ...
        xs = list(rect_src.data.get("x", [])); ys = list(rect_src.data.get("y", []))
        ws = list(rect_src.data.get("width", [])); hs = list(rect_src.data.get("height", []))
        ...
        xs = list(ell_src.data.get("x", [])); ys = list(ell_src.data.get("y", []))
        ws = list(ell_src.data.get("width", [])); hs = list(ell_src.data.get("height", []))
        for j, (cx, cy, w, h) in enumerate(zip(xs, ys, ws, hs), 1):
            if not w or not h:
                continue
        ...
        if not masks:
            preview_box.objects = [pn.pane.Markdown(
                "<span style='opacity:0.6'>No closed shapes found — draw at "
                "least one (≥3 points for lasso/polygon), then rasterise."
                "</span>")]
        ...
        status.object = (f"✅ Rasterised {len(masks)} shape(s), {total:,} px "
                         f"total. Click **▶ Send to Analysis** to stage them.")

    # -- buttons --------------------------------------------------------------
    raster_btn = pn.widgets.Button(name="▦ Rasterise drawn shapes",
    ...
    def _send(event):
        if not _draw["masks"]:
            status.object = "Rasterise first — nothing staged."
            return
        img._pending_drawn = {k: v.copy() for k, v in _draw["masks"].items()}
        status.object = (f"✅ Staged {len(_draw['masks'])} ROI(s). In the "
