...
session_name = pn.widgets.TextInput(name="Session name", value="session")
...
session_note = pn.pane.Markdown(
    ...
    "Binning is a view setting and is not saved into the session.</span>")


def _session_path():
    folder = os.path.expanduser(session_dir.value or "~")
...
def _save_session(event):
    if state.img is None:
        ...
        return
    try:
        folder, path = _session_path()
        os.makedirs(folder, exist_ok=True)
        state.img.save_session(path, verbose=False)
        mb = os.path.getsize(path) / 1e6
        session_status.object = (f"✅ Saved `{os.path.basename(path)}` "
                                 f"({mb:.1f} MB)")
...
sidebar = pn.Column(
    ...
    session_dir,
    session_name,
    pn.Row(save_session_btn, load_session_btn),
    session_status,
    session_note,
    pn.layout.Divider(),
