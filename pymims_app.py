...
try:
    ...
    try:
        ...
        _HMR_OK = False
        _HMR_ERR = f"{type(_hmr_exc2).__name__}: {_hmr_exc2}"
...
def hmr_view():
    ...
    H = {"m": None, "counts": None, "axes": {}, "axis_label": {}, "path": None,
         "fit": None, "fit_axis": None, "ident": None}
    ...
    # ── widgets: species ID ────────────────────────────────────────────────────
    id_elems = pn.widgets.TextInput(name="Expected elements",
                                    value="C,N,H,O", width=260)
    ...
    def _render_spectrum():
        ...
        if res is not None and res.n_peaks > 0 and H["fit_axis"] == ax_key:
            ...
            for p in res.peaks:
                ...
                ax.plot(mm, comp, lw=0.9, alpha=0.6, zorder=2)
                ax.axvline(p.mu, color="#d62728", ls=":", lw=0.7, zorder=1)
        # species-ID candidate labels (on the fit axis)
        ident = H["ident"]
        if ident is not None and H["fit_axis"] == ax_key:
            ymax = max(float(np.max(y)), 1.0)
            for lab in ident.labels:
                mo = lab["observed_mz"]
                ax.annotate(lab["candidate"].formula, xy=(mo, ymax),
    ...
    def _load(event=None):
        ...
        H.update(path=path, axes=axes, axis_label=labels, counts=counts,
                 m=axes.get("amu", next(iter(axes.values()))),
                 fit=None, fit_axis=None, ident=None)
    ...
    def _run_deconv(event=None):
        ...
        try:
            ...
            return
        H["fit"] = res; H["fit_axis"] = ax_key; H["ident"] = None
        # peak table
        rows = []
        ...
        try:
            ...
            dc_table.objects = [pn.pane.DataFrame(df, index=False, width=560)]
        except Exception:
            dc_table.objects = [pn.pane.Str(str(rows))]
        dc_status.object = (
    ...
    def _identify(event=None):
        ...
        if res is None or res.n_peaks == 0:
            id_out.objects = [pn.pane.Markdown(
                ...
                "fitted peak centres.")]
            return
        elems = [s.strip() for s in id_elems.value.replace(" ", ",").split(",")
                 if s.strip()]
        pol = -1 if id_pol.value.startswith("anion") else 1
        peaks_mz = [p.mu for p in res.peaks]
        try:
            match = match_by_separation(peaks_mz, elems, polarity=pol,
                                        max_atoms=int(id_maxatoms.value))
        except Exception as exc:
            id_out.objects = [pn.pane.Markdown(f"❌ Identify failed: `{exc}`")]
            return
        if match is None:
            id_out.objects = [pn.pane.Markdown(
                "No candidates — check the expected elements / polarity.")]
            return
        H["ident"] = match
        rows = []
        for lab in match.labels:
            c = lab["candidate"]
            rows.append([f"{lab['observed_mz']:.5f}", c.formula,
                         f"{c.mz:.5f}", f"{lab['error_mau']:+.2f}",
                         f"{c.plausibility:.4f}"])
        header = ["observed", "candidate", "exact m/z", "err (mau)", "plausibility"]
        objs = [pn.pane.Markdown(
            f"Anchored on **{match.anchor.formula}** "
            f"(plausibility {match.anchor.plausibility:.4f}) · "
            f"separation RMS = **{match.rms_mau:.2f} mau**")]
        try:
            import pandas as pd
            objs.append(pn.pane.DataFrame(pd.DataFrame(rows, columns=header),
                                          index=False, width=620))
        except Exception:
            objs.append(pn.pane.Str(str(rows)))
        id_out.objects = objs
        _render_spectrum()
    ...
    note = pn.pane.Markdown(
        ...
        "instrument's nominal scale, not exact mass — so identification works "
        "on <i>relative</i> peak separations: it anchors the brightest "
        "deconvolved peak to a candidate ion built from your expected elements "
        "and matches the others by separation (RMS in milli-amu). MRP uses the "
        "10–90% edge width. Sparse is the robust default; N-sweep is the manual "
        "BIC fallback.</span>")
    ...
    return pn.Column(
        ...
        pn.Row(dc_nmin, dc_nmax, dc_grid, dc_stab),
        dc_status,
        dc_table,
        resid_box,
        pn.layout.Divider(),
        ...
        pn.Row(id_elems, id_pol, id_maxatoms, id_btn),
        id_out,
        note,
    )
