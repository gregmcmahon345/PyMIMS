...
def three_d_view(img):
    ...
    def _build_pointcloud():
        ...
        fig = go.Figure(go.Scatter3d(
            x=x_um, y=y_um, z=zz.astype(float), mode="markers",
            marker=dict(size=float(msize.value), color=vals,
    ...
    def _build_isosurface():
        ...
        fig = go.Figure(go.Volume(
            x=x_um.flatten(), y=y_um.flatten(), z=z_co.flatten(),
            value=vals.flatten(), isomin=vmin, isomax=vmax,
    ...
    def _render(event):
        ...
        if fig is None:
            ...
            return
        _fig["obj"] = fig
        plot_box.objects = [pn.pane.Plotly(fig, config={"responsive": True})]
        status.object = msg
