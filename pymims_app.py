...
JOURNAL = []   # list of {'png': bytes, 'caption': str, 'added': str}
...
journal_note = pn.pane.Markdown(
    "<span style='font-size:11px;opacity:0.75'>Use “➕ Add current view to "
    "journal” on the Imaging tab to collect figures (with auto captions from "
    "the view settings), then export them here. PowerPoint lays out 6 per "
    "slide (2×3); Word stacks one per figure. Each export is stamped with the "
    "date.</span>")
...
def _update_journal_status():
    if not JOURNAL:
        ...
        return
    lines = [f"**Journal — {len(JOURNAL)} view(s):**"]
...
def _build_docx(path):
    ...
    sub = doc.add_paragraph(f"Generated {stamp}  ·  {len(JOURNAL)} figures")
    if sub.runs:
    ...
    for i, e in enumerate(JOURNAL, 1):
        ...
        for run in cap.runs:
            run.italic = True
            run.font.size = Pt(9)
        doc.add_paragraph("")     # spacer between figures
    doc.save(path)
    return path
    ...
    for s in range(n_slides):
        ...
        for k in range(per):
            ...
            pic = slide.shapes.add_picture(io.BytesIO(e['png']), left, top,
                                           width=cell_w)
            # Constrain by height if the (square-ish) figure would overflow.
            if pic.height > img_h:
...
def _export_journal(event):
    ...
    export_status.object = (f"✅ Exported {', '.join(written)} "
                            f"({len(JOURNAL)} figures) to `{folder}`")
