...
def lsf_view():
    ...
    def _compute(event=None):
        ...
        try:
            ...
            amp = peak - base
            if amp <= 0:
                status.object = "❌ Flat spectrum — no peak to analyse."
                return
            ic = int(np.argmax(ys))
            ...
            # slit / flat-top width = separation of the two edge midpoints
            ...
            # abundance sensitivity: tail level at center ± Δm, in ppm of peak
            center = float(m[ic])
            dm = float(dm_as.value) / 1000.0
            ybl = np.clip(ys - base, 0, None)
            lo = float(np.interp(center - dm, m, ybl)) / amp * 1e6
            hi = float(np.interp(center + dm, m, ybl)) / amp * 1e6
            ...
            metrics.object = (
                ...
                f"{slit_w:.3f} m{u} — this is the slit's contribution; the LSF "
                f"FWHM above is the optics' contribution.\n\n"
                f"**Abundance sensitivity** at Δm = {dm_as.value:.1f} m{u} from the "
                f"peak: low-mass side {lo:,.0f} ppm, high-mass side {hi:,.0f} ppm "
                f"(tail counts as a fraction of peak height — the skirt a minor "
                f"isotope would sit on).")
    ...
    note = pn.pane.Markdown(
        ...
        "be seen above. Series mode (metric vs tuning setting) is the planned next "
        "step.</span>")

    return pn.Column(
        intro, pn.Row(src_btn), status, pn.layout.Divider(),
        pn.Row(axis_sel, edge_sel, smooth, dm_as), pn.Row(compute_btn),
        metrics, fig_box, note, sizing_mode="stretch_width")
