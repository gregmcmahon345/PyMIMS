...
def hmr_view():
    ...
    id_maxatoms = pn.widgets.IntInput(name="Max atoms", value=4, start=1, end=6,
                                      width=110)
    id_plaus = pn.widgets.FloatSlider(
        name="Plausibility weight (mau / decade)", start=0.0, end=5.0, step=0.25,
        value=1.0, width=260)
    ...
    def _identify(event=None):
        ...
        try:
            match = match_by_separation(peaks_mz, elems, polarity=pol,
                                        ...
                                        plaus_weight=float(id_plaus.value))
        except Exception as exc:
        ...
        objs.append(_html_table(header, rows, width=680, right_from=2))
        id_out.objects = objs
        _render_spectrum()
    ...
    return pn.Column(
        ...
        pn.Row(id_pol, id_maxatoms, id_plaus, id_btn),
        id_out,
        note,
    )
...
def analysis_view(img):
    ...
    def _roi_compute(event):
        ...
        objs = []
        objs.append(pn.Column(_html_table(header, rows, width=920),
