...
def analysis_view(img):
    ...
    def _h_run(event):
        ...
        try:
            ...
            return
        fig = plt.gcf()
        _h["png"] = _fig_to_png(fig)          # also closes the figure
        _h["text"] = buf_txt.getvalue().rstrip()
        h_plot_box.objects = [pn.pane.PNG(_h["png"], width=820)]
    ...
    children += [clustering_section, pn.layout.Divider(), distributions_section]
