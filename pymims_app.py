    ...
    def _compute(event=None):
        ...
        try:
            ...
            if windowed:
                sel = (m >= lo) & (m <= hi)
                if int(sel.sum()) < 5:
                    status.object = "❌ Window is empty or too narrow — widen it."
                    return
                m, y = m[sel], y[sel]
            ...
            peaks_warn.object = (
                (f"⚠️ **{npk} peaks detected and no window set.** The edge-finder "
                 f"splits at the tallest peak, so the ESF/LSF (and the slit-width "
                 f"number) will be contaminated by the neighbour. Set a **window** "
                 f"around one *outer* edge — the rising edge of the lowest-mass "
                 f"peak, or the falling edge of the highest-mass peak — which are "
                 f"still clean single-species edges. The overlap region between "
                 f"peaks is not a valid edge.")
                if (npk > 1 and not windowed) else "")
