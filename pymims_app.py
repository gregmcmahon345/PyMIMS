...
def analysis_view(img):
    ...
    def _stats_fig(header, rows, source_label):
        ...
        fig, ax = plt.subplots(
            figsize=(min(2.0 + 1.4 * ncol, 18), 0.6 + 0.4 * nrow))
        ax.axis("off")
        ...
        ax.set_title(f"ROI statistics ({source_label}) — "
                     f"{os.path.basename(img.path)}", fontsize=10, pad=12)
        fig.tight_layout()
        return fig
    ...
    def _roi_compute(event):
        ...
        _roi["stats_png"] = _fig_to_png(_stats_fig(header, rows, source_label))
        objs = []
        try:
            import pandas as pd
            df = pd.DataFrame(rows, columns=header)
            objs.append(pn.pane.DataFrame(df, index=False, width=900))
        except Exception:
            objs.append(pn.pane.Str(_roi["stats_text"], styles={
                "font-family": "ui-monospace, Menlo, monospace",
                "white-space": "pre", "font-size": "12px"}))
        objs.append(pn.pane.PNG(_roi["stats_png"], width=860))
    ...
    roi_note = pn.pane.Markdown(
        ...
        "not a mean of per-pixel ratios. Edge/threshold detection works on one "
        "channel; cluster ROIs come straight from the clustering run above.</span>")
